...
class StitchMonster(Field):
    ...
    def __init__(
        ...
        self.mlp_head = MLP(
            ...
            + self.shared_decoders_dirctions.get_out_dim(),
        ...
        self.load_pretrain()
        # set heads
        self.field_output_density = DensityFieldHead(in_dim=self.mlp_base.get_out_dim(), activation=trunc_exp)
        self.field_output_RGB = RGBFieldHead(in_dim=self.mlp_head.get_out_dim())
        self.field_output_RGB_withoutDir = RGBFieldHead(in_dim=self.mlp_base.get_out_dim())
        self.fied_RGBTUNE = RGBTUNEHead(in_dim=self.mlp_base.get_out_dim())
        self.Linear = LinearHead(in_dim=self.direction_encoding.get_out_dim(), out_dim=93)
        # TODO 改成双线性差值
    ...
    def get_outputs(
        ...
        RGBwithoutDir = self.field_output_RGB_withoutDir(density_embedding.view(-1, self.mlp_base.get_out_dim()))
        RGBTune = self.fied_RGBTUNE(density_embedding.view(-1, self.mlp_base.get_out_dim()))
        thefinal = (1 - RGBTune) * RGBwithDir + (RGBTune * RGBwithoutDir)
        outputs[FieldHeadNames.RGB] = thefinal.view(*outputs_shape, -1).to(directions)
        outputs[FieldHeadNames.RGB0] = RGBwithDir.view(*outputs_shape, -1).to(directions)
        outputs[FieldHeadNames.RGB1] = RGBwithoutDir.view(*outputs_shape, -1).to(directions)
